...
public:
    ...
    /*
    Unlike the earlier definition of Set, we did not pass a set as a pointer to
    the functions. The reason for this is that the operands of an overloaded operator
    cannot be all pointers.
    */
...
Bool operator & (int elem, Set set)
{
    for (int i = 0; i < set.card; ++i)
        if (set.elems[i] == elem) return True;
    return False;
}
...
Bool operator == (Set set1, Set set2)
{
    ...
    for (int i = 0; i < set1.card; ++i)
        if (!(set1.elems[i] & set2)) return False;
    return True;
}

inline Bool operator != (Set set1, Set set2)
{
    return !(set1 == set2);
}
