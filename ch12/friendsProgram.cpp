...
void displayOneFriend(Friend chums);
...
void displayOneFriend(Friend chums)
{
    ...
    cout << left << setw(35) << chums.getName() << left << setw(8)
         << chums.getAge() << setw(18) << chums.getFavoriteFood();

    chums.getLotteryNumbers(n1, n2, n3);
    if (n1 != -1) { // if the lottery numbers are valid, then
        cout << "   " << n1 << " " << n2 << " " << n3 << endl;
    }
    else
